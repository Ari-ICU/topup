...
export const fulfillTransaction = async (id: string): Promise<{
    ...
    // 1. Resolve dependencies dynamically to avoid circular imports if any
    const { processTopUp } = await import("./topup-provider.service.js");

    // 2. Load transaction with package info
    const transaction = await prisma.transaction.findUnique({
        where: { id },
        include: {
            package: {
